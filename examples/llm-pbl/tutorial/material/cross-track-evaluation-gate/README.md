...
| L2 | cluster-aware exact paired test、持久化预登记与 lineage sensitivity audit、anchor drift、evaluator epoch、re-baseline 与 alpha-spending | policy 必须早于 evidence 且可重放；逐 task 显著但 source/user/domain 不显著时冻结；独立 marginal trial 再被全局预算拒绝；漂移后冻结 | ✅ [主代码](L2_evaluator_governance.py) · [主教程](tutorial_L2.md) · [敏感性代码](L2_cluster_sensitivity.py) · [敏感性教程](tutorial_L2_sensitivity.md) |
...
python3 L2_cluster_sensitivity.py
...
本地发布控制的替身。L2 的 anchors、approval ids、lineage manifest、cluster ids 与 paired deltas 也都是
synthetic：它证明的是伪重复/口径敏感性拒绝、冻结、换代、证据失效和 error-budget lineage，不证明 manifest
字段真的对应因果独立单元，也不自动裁决哪种 estimand 正确。
敏感性 companion 用真实 SQLite 顺序保存 manifest→policies→evidence→audit，拒绝 evidence 后新增 policy，
并验证 hash chain、event replay 与重启恢复；这只证明单库收到对象的顺序，不证明调用者从未提前看过 outcome，
也不提供独立时间戳、身份或不可抵赖。L3a 使用两个真实 SQLite 事务域与逻辑时钟，但 router 仍是本地服务替身；
