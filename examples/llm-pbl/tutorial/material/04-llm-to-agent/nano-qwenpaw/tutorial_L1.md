...
> 50–60% / 20% / 100–100%，并逐项对照 qwenpaw scroll 源码（`manager.py` / `eviction_index.py` / `history.py` / `cap_middleware.py`）。
...
[0] corpus: real sources -> facts + padding
    eviction_index.py    sha256[:8]=a568228b  mode=live
    ...
    conversation: 24 turns | transcript ~1754 est-tokens | model window=1000 | harness budget=1000

[1] append-only (none): recall of fact#1 as the conversation grows
    recall curve (probe fact#1 after each turn once it entered): 11111111111110000000000
    fact#1 falls out of the window after turn 15 (window=1000 est-tokens)

[2] three policies, same 24-turn conversation, 20 probe questions
    policy        detail    gist  ctx_tok  extras
    none           50.0%   60.0%     1754  transcript overflows window=1000 -> head dropped
    summarize      20.0%   20.0%      927  compressions=4 kept_turns=['-', '-', 6]
    evict-index   100.0%  100.0%      999  recalls map/fts/both=0/9/11 index_lines=8 store=24rows/6864B
...
[3] where the losses sit (per-fact detail recall under each policy)
    ...
    F6      hit        hit          hit   manager.py
    F7      hit     MISSED          hit   history.py
    F8      hit     MISSED          hit   history.py
    F9      hit     MISSED          hit   SOUL.md
    F10     hit        hit          hit   SOUL.md
    summarize: summary 445 tok <- folded 1282 tok (ratio 0.35, lossy & irreversible)
...
确定性声明：seed=42、无计时行——**同一源码快照上跑两遍，输出逐字节一致**。2026-08-06 共四次独立运行，diff 均为空。
...
- **同一段 24 轮对话**：10 条 fact 轮（study note）与 14 段 padding 轮（reading note）交错，再加 padding 尾巴。fact 是 grounding 在真实源码行上的学习笔记，padding 是真实散文（LLM-PBL README/ROADMAP + qwenpaw README 的段落）。
...
- **transcript 构造为必须溢出**：全部 24 轮约 1754 est-tokens > 1000（代码里有构造断言）——不溢出就没有可比的损失。
...
于是教程里的数字**跟着权威源码走**：如果哪天 `eviction_index.py` 把 `_TIER_CAP` 改成 12，fact#1 的文本与答案会自动跟着变（思考题 5），而不是材料里写死一个过时的 10。每个源文件读取时记录 sha256 前 8 位（本轮输出：`a568228b / 7047abe2 / ea74a331 / f1913129 / 78269f03`，全部 `mode=live`），源文件不可读时退回代码内 pinned snapshot 并把 mode 打成 `PINNED`——漂移与降级都是**可见的**。
...
`none` 就是裸拼：`SYSTEM + 全部 24 轮`，prompt 长 1754 est-tokens，模型窗口 1000——**头部 754 est-tokens（约 43% 的对话）对模型不存在**。

[1] 板块把损失过程拍了下来：从 fact#1 入场（turn 2）起，每加一轮就探测一次「fact#1 还能不能被召回」：

```text
recall curve: 11111111111110000000000
fact#1 falls out of the window after turn 15
```

13 个 1 之后断崖式归零——turn 15 之后，fact#1 所在的句子被挤出窗口，召回从 100% 瞬间变 0%。没有警告、没有日志、模型也不会说「我看不到了」：**append-only 的损失是静默的**。

[2]/[3] 给出全貌：detail 50.0%（F1–F5 全丢、F6–F10 全中）、gist 60.0%。损失谱是**按新近度单调**的——丢的永远是最早的，留的永远是最近的（self-check 第 2 条断言）。这在某些场景甚至算「可预测」，但注意它的含义：**对话的开头——通常正是任务定义、用户约束、系统原则——恰恰是最先被丢的**。
...
24 轮跑完：4 次压缩，最终 ctx=927（预算内），detail/gist 都只有 **20.0%**——比什么都不做的 append-only 还低一大截。损失在哪？

```text
kept_turns=['-', '-', 6]
```

保留的 3 个槽位里，**两个被 padding 段落（'-'）占了**，只留下 fact#6。这不是 bug，是 salience 公式的必然：10 条 fact 共享一套词汇（seq / span / turns / token / store…），互相之间 IDF 被摊低；padding 散文主题各异，稀有词多，salience 反而高。**token 稀有度与任务重要性无关**——摘要政策保住了「最显眼的」，丢掉了「最该留的」。self-check 第 5 条就是把这个失败模式测出来（「salience kept padding over facts」）。

再看 [3] 表：summarize 的损失**不是新近度单调**的——F6/F10 活着，F7/F8/F9（比 F6 更新）却死了（self-check 第 4 条断言「损失非单调」）。这是比 append-only 更糟的性质：**你无法用「旧的不重要」来安慰自己，丢什么取决于词频统计，不可预测**。
...
1282 tok 的内容被压成 445 tok 的保留集，ratio 0.35。被丢的 837 tok 没有任何副本——summarize 政策不写 store，**压缩即销毁**。
...
索引行 = seq 地址 + headline。索引本身也有界：超过 `IDX_CAP = 8` 行时，**最旧两行折叠成一条 span 行**（只显示端点 headline）——于是索引永远 ≤8 行，对话再长，索引的上下文开销也是 O(1)。最终 `index_lines=8`、`ctx=999 ≤ 1000`：窗口贴着预算但没超。
...
结果：detail **100.0%**、gist **100.0%**，且「recall agency actually fired」断言在位——100% 不是靠 transcript 没溢出混来的（ctx=999 贴着预算，20 问全走召回），是靠存储设计换来的。
...
对照 `history.py:L134`（`CREATE TABLE IF NOT EXISTS conversation_history`）与 `L198-199`（`fts5(content, content='conversation_history', content_rowid='seq', tokenize='porter unicode61')`）——**召回用的存储技术不是教学简化，就是真实实现本身**（`store=24rows/6864B`，6.7 KB 买下整段对话的无损记忆）。

---

## 8. 与 qwenpaw scroll 源码对照（2026-08-06 核验）

权威实现就在本仓库：`qwenpaw_coach/src/qwenpaw/agents/context/scroll/`。以下锚点全部当日现场核验（sha256 见 §14）：

| nano L1 | qwenpaw scroll | 核验锚点 | 差异与原因 |
|---------|----------------|----------|-----------|
| 入场即 `store.put`（write-through） | HistoryStore write-through | `history.py:L54`（「Every event the agent appends is write-through-persisted」） | 思想相同；真实系统还把 write-through 失败做成 durability 降级健康态（`history.py:L464`、`L473`「history write-through FAILED; durability degraded」）——落盘失败不是异常日志，是系统健康状态 |
| 单层索引，`IDX_CAP=8` 时最旧两行折叠成 span 行 | 分层 odometer `EvictionIndex`：每层最多 `_TIER_CAP = 10` 块，满了 carry 上层、级联进位；`compact()` 是压力阀 | `eviction_index.py:L31`、`L146-166`（carry 级联）、`L168-192`（compact） | nano 只演示「地图 + span 可寻址」的最小形态；qwenpaw 要应对**任意长**对话——span-of-spans 与 span 用同一方式折叠（自相似），索引可以一直向单层收敛 |
| `[seq N] headline` 索引行 | `Line(seq_lo, seq_hi, head, tail)`，span 行只显示端点 | `eviction_index.py:L42-63` | 两边一致：折叠只丢「显示粒度」，不丢地址 |
| recall = 索引重叠 ≥2 词元 + FTS5 fallback（harness 规则） | 模型经 `recall_history_python` / `ms.sql_query` 自主展开 | `eviction_index.py:L20-22`、`L264-267`（「Re-expand a span … ms.expand(lo, hi)」） | 真实系统里 agency 坐在 LLM 侧；nano 用确定性规则保证实验可复现 `[TODO: needs key]` |
| sqlite3 + fts5（porter unicode61，`content_rowid='seq'`） | 同 | `history.py:L87`、`L134`、`L198-199` | 存储技术逐字同构 |
| pin 第 1 轮 | `pinned: int = 1` | `manager.py:L48` | 默认值相同：头部（任务/原则）永不被逐出 |
| 超阈值 → pinned head + recent tail，中间逐出成索引 | 同 | `manager.py:L10`（「past the token threshold, keep a pinned head + recent tail」）、`L12`（「no summarization, nothing lost」）、`L142-143`、`L168-179` | 结构同构 |
...
一个值得单独点的工程细节：qwenpaw 的索引在 prompt 里是**一个占位消息**，`render()` 的 docstring 明确写了 KV-cache 安全——intro 与 `Tier N` banner 是常量，新逐出只在 Tier 0 底部**追加**块，以上字节全部不变，prefix cache 可以撑到第一个新 token（carry 重排上层才会破，那是 roll-up 的固有代价）（`eviction_index.py:L243-253`）。**索引的数据结构选择直接影响推理成本**——这与 03 轨 nano-vllm-sglang L1 讲的 prefix caching 是同一个机制的两端。
...
|------|---------------|-----------|--------------|--------|
| none | 50% / 60% | 头部（最旧）内容 | 可预测：按新近度单调 | 不可逆（没有副本） |
| summarize | 20% / 20% | salience 低的轮——**与重要性无关** | 不可预测：非单调 | 不可逆（445←1282，ratio 0.35） |
...
2. **摘要的损失谱比不管理更糟**。这反直觉（「压缩总比丢了好」），但测出来就是：20% < 50%，而且损失不可预测。根因是 salience 信号（词元稀有度）与任务信号（这条信息后面会不会被问）正交。**摘要不是无损压缩，是基于错误重要性假设的有损丢弃**。qwenpaw 的选择因此是「no summarization, nothing lost」（`manager.py:L12`）——不做摘要不是偷懒，是拒绝引入一个不可预测的损失源。
3. **记忆可靠性是存储设计，不是模型属性**。同一个 WindowModel、同一段对话，换个政策，recall 从 50% 到 100%。模型没变，变的是「什么在窗口里」——而这完全由 harness 的存储策略决定。这就是 04 轨的核心命题之一：**agent 的可靠性工程，很大一部分是上下文工程**。
...
1. **窗口算术**：transcript 1754 est-tokens、窗口 1000——`none` 政策下模型看不到的比例是多少？（754/1754 ≈ 43%。）如果把对话加长到 40 轮，recall curve 的断崖点（turn 15）会往哪移？改 `turns` 构造跑一遍验证。
...
4. **pinned 的由头**：recall 展开会把总 context 推过 WINDOW，WindowModel 的头部裁切会连 SYSTEM prompt 一起切掉（prompt_head 999 贴着预算，展开后必超）——这是声明过的 mock 性质。对照 `manager.py:L48` 的 `pinned: int = 1`：真实系统为什么要把头部 pin 住、让它永远不参与逐出？
...
3. **估计器的绝对值不可外推**。`ceil(chars/4)` 是声明估计器（真实系统用模型自己的 count_tokens，`cap_middleware.py:L24`）——1754/927/999 这些绝对值只在声明口径内成立；政策排序与损失谱的结构不依赖估计器选择。
...
**五源 sha256（2026-08-06 现场核验，`shasum -a 256`，全部 `mode=live`）**：

| 源文件 | sha256 |
|--------|--------|
| `src/qwenpaw/agents/context/scroll/eviction_index.py` | `a568228bbf5da5313c5773db5766033222d6181f6e49ebcc00b0d5a5b746a583` |
| `src/qwenpaw/agents/context/scroll/cap_middleware.py` | `7047abe268f7a233b741efee627d8412c9eee6f8dc929c1f7c53007aa9cb17f2` |
| `src/qwenpaw/agents/context/scroll/manager.py` | `ea74a331f4474f96ff6aa41e623df258835ca6a31ac98e2f19c8f0ae96f2846c` |
| `src/qwenpaw/agents/context/scroll/history.py` | `f191312956e643bf9e3aaab847cd0e589c54801650dd55cebe72ed96d97230bc` |
| `coach/profile/SOUL.md` | `78269f032c60bc142864a401ac995b0c7a578f436ff48c317d8d523510ea1be8` |

**引用锚点**（同上快照，行号级核验）：`eviction_index.py:L20-22`（nothing is lost / ms.sql_query）、`L31`（`_TIER_CAP = 10`）、`L42-63`（Line/Leaf）、`L146-166`（carry 级联）、`L168-192`（compact 压力阀）、`L243-253`（render / KV-cache 安全）、`L264-267`（ms.expand 召回说明）；`cap_middleware.py:L24`（模型自己的估计器）、`L27-28`（唯一 capping 路径、不丢数据）、`L38`（`token_cap: int = 3000`）、`L46`/`L97`（`tool_call_id` 键控）；`manager.py:L10`/`L12`（pinned head + recent tail / no summarization）、`L48`（`pinned: int = 1`）、`L142-143`/`L168-179`（trigger/split）；`history.py:L51`（HistoryStore）、`L54`（write-through）、`L87`（sqlite3.connect）、`L134`（conversation_history）、`L198-199`（fts5 porter unicode61）、`L464`/`L473`（durability 降级）。

**SOUL.md 原则数**：7 条编号原则（`## 1`–`## 7`，2026-08-06 现场 `grep -c '^## [0-9]'` = 7；principle 5 = Anti-Hallucination: Zero Tolerance，L53）。L0 教程与 README 曾写「5 原则」，属过时表述——L1 代码的 live 正则提取与本轮独立核验答案一致：**7**（K+1 / 费曼 / PBL / 对抗自检 / 反幻觉 / 学习者自主 / 持续改进），已随本轮 README 更新修订。

**确定性记录**：同一源码快照多次独立运行，输出均逐字节一致；五个来源文件的 sha256 在三次复验中零漂移（2026-08-06）。

**环境**：Apple M5 Pro / Python 3.13.13（`python3`）/ 纯标准库 / CPU / seed=42。真实托管模型：`[TODO: needs key]`。
