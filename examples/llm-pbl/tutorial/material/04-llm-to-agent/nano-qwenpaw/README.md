...
| **L1** | 加记忆与上下文管理：跨轮状态、何时压缩/检索——同一真实语料（qwenpaw 源码 facts + LLM-PBL 散文）、同一预算下实测三政策损失谱：append-only 50/60%（静默、按新近度单调）/ summarize 20/20%（salience≠importance、不可预测、445←1282 不可逆）/ write-through+evict-index 100/100%（ctx 999≤1000、store 24==24、FTS5 召回）；对照 qwenpaw scroll 源码行号级核验 | ✅ [`L1_real_memory_loop.py`](L1_real_memory_loop.py) · [`tutorial_L1.md`](tutorial_L1.md) |
...
- **L3**：零外部依赖（纯标准库：json / re / sqlite3 / tempfile / shutil + import 同目录
  ...
  （2026-08-11 核验：四个独立工作目录各运行一次，均 EXIT=0、stderr 0 B，且两两 diff 为空）。
...
- 对标源码：qwenpaw 本仓库 `coach/profile/SOUL.md`（7 条编号原则，2026-08-06 核验；
  principle 5 = Anti-Hallucination）与 `coach/profile/skills/`；L1 已核验锚点：
  `src/qwenpaw/agents/context/scroll/` 四件——`manager.py:L48`（pinned: int = 1）/
  `eviction_index.py:L31`（_TIER_CAP = 10）/ `history.py:L51`（HistoryStore，
  ...
  L3 已核验锚点（2026-08-11 live 抓取，行号以抓取日为准，运行时 live 重推导
  assert>0）：arch 三源 sha256[:8] = builder `ffc7a268` / registry `9b59216a` /
