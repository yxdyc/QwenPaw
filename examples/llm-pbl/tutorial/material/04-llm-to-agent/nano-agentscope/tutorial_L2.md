...
**参考答案**：每条“请你确认”都可能完全合法，单条 schema 与权限检查无法判错；失败只在序列层表现为状态没有推进、同一 subtask 重复、证据集不增长。守卫因此要维护 progress key、最近消息窗口和预算，把 livelock 与普通慢任务区分开。预算只能止损，模式检测才能给出可诊断的终止原因。
