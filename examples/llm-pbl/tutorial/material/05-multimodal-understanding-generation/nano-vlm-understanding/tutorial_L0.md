...
## 6. 费曼自检：答对、看图与定位是同一件事吗

1. 一个系统在 baseline 上 9/9，为什么仍不能证明它使用了图像？
2. image-swap 后答案改变，为什么也不能单独证明视觉理解正确？
3. patch-shuffle 与 remove-2D-position 的失败分别定位哪一层机制？

<details>
<summary>参考答案</summary>

1. 正确答案可能来自问题措辞、类别先验或训练集泄漏；baseline 只观测结果，没有识别答案的因果来源。至少要固定问题并删图、换图，观察预测是否随视觉证据按预期变化。
2. sensitivity 只要求两次输出不同。模型可能从错误 A 变成错误 B，因此还要同时要求 swap pair 两边各自正确，并保留 completion 和原始回答。
3. shuffle 保留位置槽却打断“内容属于哪个位置”的绑定，主要检查内容—位置配对；移除二维位置保留内容集合却抹去坐标，主要检查空间索引。两者都失败才支持“readout 需要有序的视觉 token”，仍不等于真实图像 grounding 已被证明。
