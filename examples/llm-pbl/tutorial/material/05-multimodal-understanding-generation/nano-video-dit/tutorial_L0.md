...
一段视频不等于一摞各自漂亮的图片。它更像一条物体轨迹：每一帧既要成立，还要与前后时刻连续。把 2D DiT 扩到视频时，新增的时间轴同时带来建模约束和二次增长的注意力账单。

> **核心问题**：加入时间坐标和首尾帧条件后，联合时空预测怎样减少轨迹抖动，完整注意力成本又如何随帧数增长？
> **先修**：image DiT L0 的 latent token、位置编码、flow 与 Euler 更新。
> **运行**：`python3 -B L0_spatiotemporal_latent_dit.py`，纯标准库，CPU 秒级。
> **验收**：首尾帧误差保持为零；joint predictor 的 roughness/flicker 优于逐帧反例；token 与 $N^2$ 关系账可复算。
> **边界**：中间轨迹由确定性教学函数构造，没有训练网络，也没有证明真实视频质量或 kernel 延迟。
...
## 6. 费曼自检：视频质量为什么不能化约为逐帧质量

1. 两个预测器的首尾帧误差都为零，为什么 joint 仍更好？
2. roughness 为零是否总是好视频？请给一个反例。
3. 帧数翻倍时 full-attention pairs 约增 4×，为什么真实延迟未必正好增 4×？

<details>
<summary>参考答案</summary>

1. endpoint 只约束两个时刻，对中间路径没有识别力。independent 轨迹在端点之间来回跳，二阶差分和速度偏差都更高；joint 条件让各帧共享同一运动假设，所以本例的中间状态一致。
2. 不是。静止画面或匀速插值都可得到零 roughness，却可能违背“突然起跳”“镜头切换”等提示，也可能把运动物体冻结。平滑度必须与事件、提示、外观和物理约束共同报告。
3. $N^2$ 是注意力关系数量。端到端还包括 VAE、MLP、调度步数和 I/O，kernel 可能从低利用率进入高利用率，也可能触发显存溢出、tiling 或通信。因此复杂度给增长机制，真机测量才给该栈上的延迟。
