...
Qwen-Image-2512 是当前预定 DiT 真实开放基线。Qwen-Image-2.0 已有更新的官方模型与技术报告，但截至 2026-09-15，
官方开放仓库尚未给出 2.0 本地权重 quick start，因此只进入前沿追踪。

[HunyuanImage-3.0](https://github.com/Tencent-Hunyuan/HunyuanImage-3.0) 已开放 Base/Instruct/Distil 权重，但它是 80B total、
13B active 的原生多模态**自回归**生图/编辑路线，不是 DiT 的新版本。课程将它用作架构反例：
说明当前生图并非只有 VAE+flow 一条路；本地实验必须另过显存、并行、revision 和许可 gate。
...
- [超长上下文与 RAG](../LONG_CONTEXT_OR_RAG.md)：多页图像/编辑历史如何消耗 context budget。
- [上级研究账本](../RESEARCH.md)
