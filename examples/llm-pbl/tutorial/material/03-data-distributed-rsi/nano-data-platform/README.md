...
> **抓的核心机制**：数据接入 → 分层存储（raw/curated）→ 治理（成本/权限/质量）→ 训练/检索消费（ROADMAP §七）。
...
L0 的选择是把「分层契约」和「声明式状态管理」这两个最本质的机制裸出来：不碰 HCL、不碰真实存储、不碰云 API——它们是语法和环境，不是机制（ROADMAP §七 写作原则）。
