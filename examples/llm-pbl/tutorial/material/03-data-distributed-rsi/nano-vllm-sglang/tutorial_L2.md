...
连续 KV 预留像要求每位住客一开始就包下一整层酒店，即使他只住一晚。分页把楼层切成等大的房间，请求按需领取，再用 block table 记住这些房间在逻辑上如何连成一条序列。

> **核心问题**：物理块池、block table、refcount、写时复制和抢占如何共同提高准入率，同时保持生成语义不变？
> **先修**：L0 的分页算术与 L1 的 KV cache、batching、调度成本。
> **运行**：`python3 -B L2_real_paged_memory.py`，依赖 torch 与同目录 L1，CPU 约 1 分钟。
> **验收**：分页与连续实现逐 token 一致，块数始终满足 `ceil(tokens / block_size)`，并在同一容量下准入更多请求。
> **边界**：物理块管理是可执行的最小同构实现；权重仍是 L1 随机初始化小模型，未测真实 GPU 引擎吞吐。

L2 还加入内容哈希前缀缓存、CoW 与抢占/重算。它们是否正确，最终都要回到同一条机器断言：内存布局可以改变，生成 token 不能改变。
