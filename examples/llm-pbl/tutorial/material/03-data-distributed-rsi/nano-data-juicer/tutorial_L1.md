...
1. 若 scorer 只读样本且后续 filter 不依赖 scorer 副作用，最终保留集合可以相同；成本不同：先过滤只需评 7 条，顺序颠倒要评 10 条。便宜、确定、召回高的规则门应尽量前置，把昂贵判断留给小集合。
2. 长度是可测 proxy，只有在固定任务/语言/格式、短答案确实意味着缺步骤，且已用人工标注校准时才合理。它看不见事实正确性、重复啰嗦、风格、推理有效性和对抗填充；跨域后相关性可能反转。
3. 10 万条串行调用会遇到吞吐、费用、超时、限流、重试重复计费与模型版本漂移。需要 batch/async、幂等缓存、速率限制、失败分类、checkpoint 和版本化输出；Ray 只提供并行执行基底，不自动解决 provider 配额与 judge 有效性。
