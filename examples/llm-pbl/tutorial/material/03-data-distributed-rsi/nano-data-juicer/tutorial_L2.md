...
分布式数据流水线像把一摞试卷分给多位批改者：格式化、长度过滤可以各做一摞；全局去重却必须在某处汇总，否则分在不同人手里的重复答案永远不会相遇。

> **核心问题**：哪些 OP 能独立分区，哪些需要全局收敛点，失败后又怎样只重算必要部分？
> **先修**：[L0](tutorial_L0.md) 的 OP 组合、[L1](tutorial_L1.md) 的真实数据/LLM OP，以及 [nano-ray L0](../nano-ray/tutorial_L0.md) 的任务图。
> **运行**：`python3 -B L2_distributed_pipeline.py`，纯标准库 multiprocessing，4 个真实 worker 进程。
> **验收**：串行与并行输出逐位一致，漏斗固定为 `3360 → 2358 → 2110`；块内错误去重的反例必须被抓住。
> **边界**：语料为固定 seed 的合成大样本；本级测单机多进程的执行语义，不代表真实多机 Ray 或 GPU 性能。

L0/L1 的 `op(list) -> list` 让全量数据停留在一个进程。本级只改变执行计划和成本，数据语义仍由同一组 OP 定义。
