...
object store 可以先想成一座共享仓库：`put` 把货物入库，ObjectRef 是提货单，worker 尽量在货物附近开工。这个类比只有在容量、只读、引用计数和逐出规则都能解释时才算成立。

> **核心问题**：对象实际住在哪里，零拷贝为何要求只读，容量不足时谁被逐出，嵌套 ObjectRef 又在哪一层解开？
> **先修**：L1 的传值/传引用成本账与 L2 的 actor 状态模型。
> **运行**：`python3 -B L3_object_store_zero_copy.py`，复用前两级的语料构造与漏斗断言。
> **验收**：解释并测量共享内存、字节账、顶层/嵌套解引用、逐出与 locality；数据漏斗仍为 `3360 → 2358 → 2110`。
> **边界**：结果针对给定 Ray 快照和单机 object store；多节点网络传输、故障恢复与生产容量规划仍需另测。

L1/L2 只把 store 当作成本项。本级把它当作有物理位置、容量策略和生命周期的实体，并对照源码定位依赖解析路径。
