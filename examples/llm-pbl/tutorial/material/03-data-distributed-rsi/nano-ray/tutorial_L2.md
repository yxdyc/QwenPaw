...
普通 task 像临时旅馆房间：调用结束，房里的局部状态也消失。actor 给状态一个长期地址，多个生产者可以找到同一份索引；代价是我们必须明确并发、顺序和恢复语义。

> **核心问题**：全局去重索引应当住在哪里，才能跨批次共享，同时不让到达顺序偷偷改变结果？
> **先修**：L1 的真实 Ray pipeline、ObjectRef 和全局收敛点。
> **运行**：`python3 -B L2_actor_dedup_index.py`，复用 L1 的语料与 OP。
> **验收**：漏斗保持 `3360 → 2358 → 2110`；first-seen 的顺序敏感反例必须翻转 236 条，`min-row_id` 三种投递顺序逐位一致。
> **边界**：实验验证单机 actor 的状态与并发语义；未覆盖 actor 崩溃恢复、集群迁移和长期持久化。

本级直接 import L1 的语料构造、OP 与 `EXPECTED_*`，在 [0] 用机器断言复验跨模块契约。actor 提供状态的住所，去重的正确规则仍由执行计划定义。
