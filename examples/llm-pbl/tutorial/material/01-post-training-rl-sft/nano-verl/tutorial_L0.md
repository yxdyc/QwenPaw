...
1. rollout 是逐 token 解码，KV 占用和完成长度随请求变化，适合动态批处理；train 是规则张量上的大矩阵计算，batch 受显存、梯度累积和全局优化口径约束。二者的最佳 batch size 来自不同瓶颈。
2. 当两段等长且能完全重叠时，上界是 2 倍。实测只有 1.45 倍，说明同步、启动、队列空转和有限批次让 overlap 不完整；流水线填充与排空也无法被隐藏。
3. 应比较 $T_{sync}$ 与可隐藏的 $\min(T_{roll},T_{train})$。若同步和调度开销大于被重叠掉的时间，或严格 on-policy 迫使两段频繁等待，拆分会更慢；应降低同步频率、改变拓扑或保持串行。
